  ...
  def __init__(self, slip_prob = 0.1):
    ...
    for s in range(self.rows * self.cols):
      if(s == goal_state):
        ...
        # STUDENT CODE HERE
        row, col = self.map_state_to_row_col(s)
        ...
        if (row == 0):
          ...
          next_state = self.map_row_col_to_state(row - 1, col)
          P[s][0] = [(1.0 - slip_prob, next_state, cost),
                    (slip_prob, s, cost)]
        ...
        if (col == self.cols - 1):
          ...
          next_state = self.map_row_col_to_state(row, col + 1)
          P[s][1] = [(1.0 - slip_prob, next_state, cost),
                    (slip_prob, s, cost)]
        ...
        if (row == self.rows - 1):
          ...
          next_state = self.map_row_col_to_state(row + 1, col)
          P[s][2] = [(1.0 - slip_prob, next_state, cost),
                    (slip_prob, s, cost)]
        ...
        if (col == 0):
          ...
          next_state = self.map_row_col_to_state(row, col - 1)
          P[s][3] = [(1.0 - slip_prob, next_state, cost),
                    (slip_prob, s, cost)]
