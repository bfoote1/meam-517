...
def add_continuity_constraints(prog, sigma, n, d, dt):
  ...
  for i in range(n - 1):
        for k in range(1, 5):
            expression = np.zeros(2, dtype=object)

            for j in range(k, d):
                expression += ((factorial(j) / factorial(j-k)) * dt[i]**(j-k) * sigma[i][j])
            
            expression -= factorial(k) * sigma[i+1][k]

            prog.AddLinearEqualityConstraint(expression, np.zeros(2))
...
def add_minsnap_cost(prog, sigma, n, d, dt):
  ...
  for i in range(n):
    ...
    for j in range(4, d):
        for l in range(4,d):
            h_jl = (factorial(j) * factorial(l) / (factorial(j-4) * factorial(l-4))) * ((dt[i]**(j+l-7)) / (j+l-7))
            cost += h_jl * np.dot(sigma[i][j], sigma[i][l])
