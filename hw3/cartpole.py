...
class Cartpole(object):
  ...
  def compute_energy_shaping_input(self, t, x):
    ...
    q1_ddot_des = qdot[1] * np.cos(q[1]) * e_tilde
    ...
    u = (self.mc + self.mp - self.mp * np.cos(q[1])**2) * q1_ddot_des - self.mp * self.g * np.sin(q[1]) * np.cos(q[1]) - self.mp * self.L * np.sin(q[1]) * qdot[1]**2
